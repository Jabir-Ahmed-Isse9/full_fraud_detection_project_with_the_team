    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-slate-900 text-white p-3 rounded-lg shadow-xl text-xs font-mono border border-slate-800">
          <div className="font-semibold text-blue-400">{label} Distribution</div>
          <div className="text-slate-300 mt-1">
            Total Records: <span className="text-white font-bold">{formatNumber(item.count)}</span>
          </div>
          <div className="text-slate-400">
            Share of Dataset: <span className="text-slate-200 font-bold">{item.percentage}%</span>
          </div>
