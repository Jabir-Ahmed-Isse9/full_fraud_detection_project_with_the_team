...
export default function TransactionAmountChart({ data = [] }) {
  if (!data || data.length === 0) {
    return (
      <div className="h-64 flex items-center justify-center text-xs text-slate-400">
        No transaction amount distribution data
      </div>
    );
  }
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-slate-900 text-white p-3 rounded-lg shadow-xl text-xs font-mono border border-slate-800">
          <div className="font-semibold text-blue-400">Amount Tier: {label}</div>
